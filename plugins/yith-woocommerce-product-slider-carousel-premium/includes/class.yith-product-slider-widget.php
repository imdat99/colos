...
if ( ! class_exists( 'YITH_Product_Slider_Widget' ) ) {
	...
	class YITH_Product_Slider_Widget extends WP_Widget {
		...
		public function form( $instance ) {
			...
			<p>
				<label
					for="<?php echo esc_attr( $this->get_field_id( 'slider_id' ) ); ?>"><?php _e( 'Select a slider', 'yith-woocommerce-product-slider-carousel' ); ?></label>
				<select id="<?php esc_attr( $this->get_field_id( 'slider_id' ) ); ?>"
						name="<?php echo esc_attr( $this->get_field_name( 'slider_id' ) ); ?>">
					<option
						value="" <?php selected( '', $instance['slider_id'] ); ?>><?php _e( 'Select a slider', 'yith-woocommerce-product-slider-carousel' ); ?></option>
					<?php foreach ( $product_sliders as $slider ): ?>
						<option
							value="<?php echo esc_attr( $slider->ID ); ?>" <?php selected( $slider->ID, $instance['slider_id'] ); ?>><?php echo $slider->post_title; ?></option>
					<?php endforeach; ?>
				</select>
			</p>
			<p>
				<label
					for="<?php echo esc_attr( $this->get_field_id( 'images_for_row' ) ); ?>"><?php echo __( 'Images for row', 'yith-woocommerce-product-slider-carousel' ); ?></label>
				<input type="number" step="1" min="1" value="<?php echo $instance['images_for_row']; ?>"
					   name="<?php echo esc_attr( $this->get_field_name( 'images_for_row' ) ); ?>">
			</p>
			<p>
				<label
					for="<?php echo esc_attr( $this->get_field_id( 'items_in_desktop' ) ); ?>"><?php echo __( 'Items in Desktop', 'yith-woocommerce-product-slider-carousel' ); ?></label>
				<input type="number" step="1" min="1" value="<?php echo $instance['items_in_desktop']; ?>"
					   name="<?php echo esc_attr( $this->get_field_name( 'items_in_desktop' ) ); ?>">
			</p>
			<p>
				<label
					for="<?php echo esc_attr( $this->get_field_id( 'items_in_tablet' ) ); ?>"><?php echo __( 'Items in tablet', 'yith-woocommerce-product-slider-carousel' ); ?></label>
				<input type="number" step="1" min="1" value="<?php echo $instance['items_in_tablet']; ?>"
					   name="<?php echo esc_attr( $this->get_field_name( 'items_in_tablet' ) ); ?>">
			</p>
			<p>
				<label
					for="<?php echo esc_attr( $this->get_field_id( 'items_in_mobile' ) ); ?>"><?php echo __( 'Items in mobile', 'yith-woocommerce-product-slider-carousel' ); ?></label>
				<input type="number" step="1" min="1" value="<?php echo $instance['items_in_mobile']; ?>"
					   name="<?php echo esc_attr( $this->get_field_name( 'items_in_mobile' ) ); ?>">
			</p>
			<p>
				<label
					for="<?php echo esc_attr( $this->get_field_id( 'anim_in' ) ); ?>"><?php _e( 'Animation In', 'yith-woocommerce-product-slider-carousel' ); ?></label>
				<select id="<?php echo esc_attr( $this->get_field_id( 'anim_in' ) ); ?>"
						name="<?php echo esc_attr( $this->get_field_name( 'anim_in' ) ); ?>">
					<option
						value="" <?php selected( '', $instance['anim_in'] ); ?>><?php _e( 'Select an animation', 'yith-woocommerce-product-slider-carousel' ); ?></option>
					<?php foreach ( $animations as $animation => $it ) : ?>
						<optgroup label="<?php echo $animation; ?>">
							<?php foreach ( $animations[ $animation ] as $key ): ?>
								<option
									value="<?php echo esc_attr( $key ); ?>" <?php selected( $key, $instance['anim_in'] ); ?>><?php echo $key; ?></option>
							<?php endforeach; ?>
						</optgroup>
					<?php endforeach; ?>
				</select>
			...
			<p>
				<label
					for="<?php echo esc_attr( $this->get_field_id( 'anim_out' ) ); ?>"><?php _e( 'Animation Out', 'yith-woocommerce-product-slider-carousel' ); ?></label>
				<select id="<?php echo esc_attr( $this->get_field_id( 'anim_out' ) ); ?>"
						name="<?php echo esc_attr( $this->get_field_name( 'anim_out' ) ); ?>">
					<option
						value="" <?php selected( '', $instance['anim_out'] ); ?>><?php _e( 'Select an animation', 'yith-woocommerce-product-slider-carousel' ); ?></option>
					<?php foreach ( $animations as $animation => $it ) : ?>
						<optgroup label="<?php echo $animation; ?>">
							<?php foreach ( $animations[ $animation ] as $key ): ?>
								<option
									value="<?php echo esc_attr( $key ); ?>" <?php selected( $key, $instance['anim_out'] ); ?>><?php echo $key; ?></option>
							<?php endforeach; ?>
						</optgroup>
					<?php endforeach; ?>
				</select>
			...
			<p>
				<label
					for="<?php echo esc_attr( $this->get_field_id( 'hide_add_to_cart' ) ); ?>"><?php _e( 'Hide "Add to cart"', 'yith-woocommerce-product-slider-carousel' ); ?></label>
			...
			<p>
				<label
					for="<?php echo esc_attr( $this->get_field_id( 'hide_price' ) ); ?>"><?php _e( 'Hide product price', 'yith-woocommerce-product-slider-carousel' ); ?></label>
